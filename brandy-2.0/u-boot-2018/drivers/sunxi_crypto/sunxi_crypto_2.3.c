		     ...
		     u8 *p_tmp, u8 *hash, u32 hash_byte_len)
{
	...
	if (!memcmp(params->r, all_zero, params->r_len) ||
		...
		return -1;
	//n
	__rsa_padding(p_tmp, params->n, params->n_len, params->n_len);
	p_tmp += key_byte_size;

	//s
	__rsa_padding(p_tmp, params->s, params->s_len, params->s_len);
	p_tmp += key_byte_size;

	//hash
	__rsa_padding(p_tmp, hash, hash_byte_len, hash_byte_len);

	p_tmp += key_byte_size;

	//r
	__rsa_padding(p_tmp, params->r, params->r_len, params->r_len);
	p_tmp += key_byte_size;

	//p
	__rsa_padding(p_tmp, params->p, params->p_len, params->p_len);
	p_tmp += key_byte_size;

	//a
	__rsa_padding(p_tmp, params->a, params->a_len, params->a_len);
	p_tmp += key_byte_size;

	//gx
	__rsa_padding(p_tmp, params->gx, params->gx_len, params->gx_len);
	p_tmp += key_byte_size;

	//gy
	__rsa_padding(p_tmp, params->gy, params->gy_len, params->gy_len);
	p_tmp += key_byte_size;
	...
	//qy
	__rsa_padding(p_tmp, params->qy, params->qy_len, params->qy_len);
	p_tmp += key_byte_size;
...
#define BUFF_LEN (68)
	...
	memset((void *)(temp_src_addr), 0, ALIGN(BUFF_LEN * 12, CACHE_LINE_SIZE));
	memset((void *)(temp_dst_addr), 0, ALIGN(BUFF_LEN, CACHE_LINE_SIZE));
