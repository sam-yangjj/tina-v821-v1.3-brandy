...
int sunxi_sha_calc(u8 *dst_addr, u32 dst_len, u8 *src_addr, u32 src_len)
{
	...
	flush_cache(((u32)&task0), ALIGN(sizeof(task0), CACHE_LINE_SIZE));
	flush_cache((u32)p_sign, CACHE_LINE_SIZE);
	flush_cache(ALIGN_DOWN((u32)src_addr, CONFIG_SYS_CACHELINE_SIZE),
		    ALIGN(src_len, CACHE_LINE_SIZE));
		      ...
		      const struct ecc_curve_param_t *ecc_param)
{
	...
	uint8_t *p;
	uint8_t idx;
	ALLOC_CACHE_ALIGN_BUFFER(u8, p_n, ALIGN(TEMP_BUFF_LEN * 8, CACHE_LINE_SIZE));
	ALLOC_CACHE_ALIGN_BUFFER(u8, p_dst, ALIGN(TEMP_BUFF_LEN * 2, CACHE_LINE_SIZE));
...
#define INPUT_PARAM(label, data)                                               \
	...
	flush_cache((u32)&task0, ALIGN(sizeof(task0), CACHE_LINE_SIZE));
	flush_cache((u32)p_n, ALIGN(TEMP_BUFF_LEN * 8, CACHE_LINE_SIZE));
	flush_cache((u32)p_dst, ALIGN(TEMP_BUFF_LEN * 2, CACHE_LINE_SIZE));
			    ...
			    uint32_t symmetric_ctl, uint32_t c_ctl)
{
	...
	flush_cache((u32)&task0, ALIGN(sizeof(task_queue), CACHE_LINE_SIZE));
