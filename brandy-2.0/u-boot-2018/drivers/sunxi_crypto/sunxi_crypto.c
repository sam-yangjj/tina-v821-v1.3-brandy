			    ...
			    uint32_t symmetric_ctl, uint32_t c_ctl)
{
	...
	flush_cache((ulong)&task0, ALIGN(sizeof(task_queue), CACHE_LINE_SIZE));
...
int sunxi_md5_calc(u8 *dst_addr, u32 dst_len, u8 *src_addr, u32 src_len)
{
	...
	flush_cache((ulong)&task0, ALIGN(sizeof(task0), CACHE_LINE_SIZE));
	flush_cache((ulong)p_sign, CACHE_LINE_SIZE);
	flush_cache((ulong)src_addr, ALIGN(src_align_len, CACHE_LINE_SIZE));
		   ...
		   u32 dst_len, u8 *src_addr, u32 src_len)
{
	...
	flush_cache((ulong)&task0, ALIGN(sizeof(task0), CACHE_LINE_SIZE));
	flush_cache((ulong)p_n, ALIGN(mod_size_len_inbytes, CACHE_LINE_SIZE));
	flush_cache((ulong)p_e, ALIGN(mod_size_len_inbytes, CACHE_LINE_SIZE));
	flush_cache((ulong)p_src, ALIGN(mod_size_len_inbytes, CACHE_LINE_SIZE));
	flush_cache((ulong)p_dst, ALIGN(mod_size_len_inbytes, CACHE_LINE_SIZE));
