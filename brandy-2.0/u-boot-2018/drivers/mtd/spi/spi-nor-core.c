...
struct nor_protection esmt_protection_32M[] = {
	{ .boundary = 0, .bp = 0, .flag = 0 },
	{ .boundary = BP_SZ_64K, .bp = BIT(0), .flag = SET_TB },
	{ .boundary = BP_SZ_128K, .bp = BIT(1), .flag = SET_TB },
	{ .boundary = BP_SZ_256K, .bp = BIT(1) | BIT(0), .flag = SET_TB },
	...
	{ .boundary = BP_SZ_8M, .bp = BIT(3), .flag = SET_TB },
	{ .boundary = BP_SZ_16M, .bp = BIT(3) | BIT(0), .flag = SET_TB },
	{ .boundary = BP_SZ_32M, .bp = BIT(3) | BIT(2) | BIT(1), .flag = SET_TB },
};

struct nor_protection py_protection[] = {
    { .boundary = 0, .bp = 0, .flag = 0 },
    { .boundary = BP_SZ_256K, .bp = BIT(3) | BIT(0), .flag = 0 },
    { .boundary = BP_SZ_512K, .bp = BIT(3) | BIT(1), .flag = 0 },
    { .boundary = BP_SZ_1M, .bp = BIT(3) | BIT(1) | BIT(0), .flag = 0 },
...
static int puya_config_cmp(struct spi_nor *nor, int cmp)
{
	int ret = 0, sr2_old, sr2_new;
		...
		loff_t *ofs, uint64_t *len)
{
	...
	if (JEDEC_MFR(nor->info) == SNOR_MFR_MACRONIX) {
		...
		mask =  EON_TB | SR_BP3 | SR_BP2 | SR_BP1 | SR_BP0;
		if (nor->size == BP_SZ_16M) {
			flash_protection = esmt_protection;
			flash_protection_size = ARRAY_SIZE(esmt_protection);
		} else if (nor->size == BP_SZ_32M) {
			flash_protection = esmt_protection_32M;
			flash_protection_size = ARRAY_SIZE(esmt_protection_32M);
		} else {
		...
		uint64_t len, int lock)
{
	...
	if (JEDEC_MFR(nor->info) == SNOR_MFR_MACRONIX) {
		...
		flash_protection = py_protection;
		flash_protection_size = ARRAY_SIZE(py_protection);
		mask = SR_BP4 | SR_BP3 | SR_BP2 | SR_BP1 | SR_BP0;
	} else {
	...
	if (lock) {
		...
		for (i = 1; i < flash_protection_size; i++) {
			if (mtd->size == flash_protection[i].boundary) {
				protect_flag = flash_protection[i].flag;
				break;
			}
		}
...
set_status:
	if (JEDEC_MFR(nor->info) == SNOR_MFR_MACRONIX) {
		...
		/**
		 * Only WPS = 0, PUYA Flash will use CMP && BP[0:4] to
		 * protect a specific area of the memory array.
		 * So do nothing whenm individual lock is enable
		 * */
		if (sunxi_individual_lock_is_enable(nor)) {
			dev_warn(nor->dev, "[PUYA]-Individual lock is enable, return\n");
			return 0;
		}

		if (protect_flag & SET_CMP)
			ret = puya_config_cmp(nor, 1);
		else
			ret = puya_config_cmp(nor, 0);
		if (ret)
			return ret;
		...
		if (status_old != status_new) {
			ret = write_sr_and_check(nor, status_new, mask);
			if (ret) {
				dev_err(nor->dev, "[PUYA]-write SR failed, ret=%d\n", ret);
				return ret;
			}
		}
		dev_dbg(nor->dev, "[PUYA]-sr:0x%x --> 0x%x\n", status_old, status_new);
	}
...
static int macronix_quad_enable(struct spi_nor *nor)
{
	...
	if (!(ret > 0 && (ret & SR_QUAD_EN_MX))) {
		dev_err(nor->dev, "Macronix Quad bit not set:%x\n", ret);
		return -EINVAL;
	}
			       ...
			       struct spi_nor_flash_parameter *params)
{
	...
	if (params->hwcaps.mask & (SNOR_HWCAPS_READ_QUAD |
		...
		case SNOR_MFR_XMC:
			...
			break;
#endif
#if defined(CONFIG_SPI_FLASH_GIGADEVICE) || defined(CONFIG_SPI_FLASH_ADESTO) || defined(CONFIG_SPI_FLASH_PUYA) || defined(CONFIG_SPI_FLASH_ZB)
		case SNOR_MFR_GIGADEVICE:
...
int spi_nor_scan(struct spi_nor *nor)
{
	...
	/**
	 * To determine if flash is in a write-protected state,
	 * requiring the reading of some status registers and printing them.
	 * */
	sr  = read_sr(nor);
	sr2 = read_sr2(nor);
	sr3 = read_sr3(nor);
	printf("SF: sr: 0x%02x, sr2: 0x%02x, sr3: 0x%02x\n", sr, sr2, sr3);

#if defined(CONFIG_SPI_SAMP_DL_EN) || defined(CONFIG_SPI_SAMP_DL_EN)
	/* To ensure compatibility with various size of nor flash,
	 * using 3-byte address to read boot_param region*/
	nor->addr_width = 3;
	...
	if (!(get_boot_work_mode() == WORK_MODE_USB_PRODUCT ||
		...
		nor->read(nor,
			  sunxi_flashmap_offset(FLASHMAP_SPI_NOR, BOOT_PARAM)
				  << 9,
			  BOOT_PARAM_SIZE, (u_char *)boot_param);
	}
	nor->addr_width = 0;
#endif
	...
	if (spi->mode & SPI_RX_QUAD && info->flags & SPI_NOR_QUAD_READ) {
		...
		if (spi->mode & SPI_IO_MODE) {
			if (spi->mode & SPI_DTR_MODE)
		...
		if (spi->mode & SPI_IO_MODE) {
			if (spi->mode & SPI_DTR_MODE)
				hwcaps.mask |= SNOR_HWCAPS_READ_1_2_2_DTR;
			else
				hwcaps.mask |= SNOR_HWCAPS_READ_1_2_2;
		}

	}
	if (spi->mode & SPI_RX_OCTAL && info->flags & SPI_NOR_OCTAL_READ) {
		...
		if (spi->mode & SPI_TX_OCTAL) {
			hwcaps.mask |= SNOR_HWCAPS_PP_1_1_8;
			if (spi->mode & SPI_IO_MODE)
				hwcaps.mask |= SNOR_HWCAPS_PP_1_8_8;
		}
	}
...
init_die:
	...
	if (get_boot_work_mode() == WORK_MODE_USB_PRODUCT ||
		...
		if (boot_param) {
			spif_set_right_delay_para(mtd, boot_param);
	...
	if (get_boot_work_mode() == WORK_MODE_USB_PRODUCT ||
		...
		if (boot_param) {
			sunxi_set_right_delay_para(mtd, boot_param);
			free_align(boot_param);
		}
	}
#else
	...
	printf("SF: Detected %s(%s %s) with page size ",
			nor->name,
			spi->mode & SPI_IO_MODE && info->flags & USE_IO_MODE ? "IO" : "",
			spi->mode & SPI_DTR_MODE && info->flags & USE_RX_DTR ? "DTR" : "");
