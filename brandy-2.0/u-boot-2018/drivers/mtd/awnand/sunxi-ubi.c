...
static int init_nand_mbr(sunxi_mbr_t *sunxi_mbr, struct ubi_mbr *nand_mbr)
{
	...
	for (i = 0; i < sunxi_mbr->PartCount; i++) {
		...
		char *partName = (char *)nand_mbr->vols[i + 1].name;
		unsigned int partSects = nand_mbr->vols[i + 1].sects;
		unsigned int bootSysPartSize = to_bytes(partSects);
		if (!strcmp(partName, "boot") || !strcmp(partName, "boot_backup")) {
			if (bootsize < bootSysPartSize + phy_blk_bytes) {
				pr_info("The CONFIG_BIMAGE_SIZE is %d bytes, The sys_partinion boot size is %d bytes\n", bootsize, bootSysPartSize);
				pr_err("[ERROR]: Must reserve bad blocks for the boot/boot_backup partition\n");
				return -EINVAL;
			} else if (bootsize < bootSysPartSize + 3 * phy_blk_bytes) {
				pr_warn("[WARN]: recommend reserving three bad blocks for the boot/boot_backup partition\n");
			}
		}
#endif
...
#if IS_ENABLED(CONFIG_SUNXI_PRIVATE_UBIDEV)
static int ubi_detach_mtd_do(void)
{
	int ret;
	char cmd[2][20];
	char *argv[2];
	...
	memset(cmd, 0x00, sizeof(cmd));
	sprintf(&cmd[0][0], "ubi");
	sprintf(&cmd[1][0], "detach");

	/* ubi part [part] [offset] */
	ret = sunxi_do_ubi(0, 2, argv);
	if (ret)
		pr_err("ubi detach err !\n");
...
uint64_t spinand_sys_part_offset(void)
{
	...
	if (fdt_path_offset(working_fdt, "sunxi_flashmap/nand_map") < 0) {
#if IS_ENABLED(CONFIG_AW_MTD_SPINAND_FASTBOOT)
...
#if IS_ENABLED(CONFIG_SUNXI_PRIVATE_UBIDEV)
	offset += info->block_size(chip) * CONFIG_SUNXI_PRIVATE_UBIDEV_BLKS;
#endif
	} else {
		unsigned int i = 0, block_num;
		char fdt_node_name[35];
		int fdt_off;

		while (1) {
			snprintf(fdt_node_name, 35,
				"sunxi_flashmap/nand_map/partition%d", i);
...
#if IS_ENABLED(CONFIG_AW_MTD_SPINAND_FASTBOOT)
	unsigned int bootsize = CONFIG_BIMAGE_SIZE * 1024 * 1024; // boot.img default define 20MiB
	if (bootsize & (phy_blk_bytes - 1))
		pr_warn("[WARN]: CONFIG_BIMAGE_SIZE should align to block size\n");
...
#if IS_ENABLED(CONFIG_SUNXI_PRIVATE_UBIDEV)
	/* private area */
	snprintf((char *)&mtd_part.name, PART_NAME_MAX_SIZE, "private_ubidev");
	mtd_part.offset = offset;
	mtd_part.bytes = info->block_size(chip) * CONFIG_SUNXI_PRIVATE_UBIDEV_BLKS;
	offset += mtd_part.bytes;
	wlen += snprintf(ubinfo->mtdparts + wlen, 512 - wlen,
...
#if IS_ENABLED(CONFIG_SUNXI_PRIVATE_UBIDEV)
int spinand_ubi_detach_mtd(void)
{
	if (ubi_detach_mtd_do())
		return -EINVAL;

	return 0;
}

int spinand_private_ubidev_attach(void)
{
	int ret;
	char cmd[6][20];
	char *argv[6];
	...
	sprintf(&cmd[0][0], "ubi");
	sprintf(&cmd[1][0], "part");
	sprintf(&cmd[2][0], "private_ubidev");

	/* ubi part [part] [offset] */
	ret = sunxi_do_ubi(0, 3, argv);
	if (ret)
		pr_err("ubi attach private_ubidev err !\n");

	return ret;
}

int spinand_download_private_ubidev(unsigned int len, void *buf)
{
	struct aw_spinand *spinand = get_spinand();
	struct aw_spinand_chip *chip = spinand_to_chip(spinand);
	struct aw_spinand_info *info = chip->info;
	...
	ret = spinand_ubi_detach_mtd();
	if (ret) {
		pr_err("ubi detach err !\n");
		goto err;
	}

	ret = spinand_private_ubidev_attach();
	if (ret)
		goto err;

	strcpy(vol.name, "private_ubidev");
	vol.type = DYNAMIC_VOL;
	/* Becausr ubi needs to reserve some blocks for normal operation,
	 * the actual available volume size is 90% of the reserved space */
	vol.sects = to_sects(info->block_size(chip) * CONFIG_SUNXI_PRIVATE_UBIDEV_BLKS / 10 * 9);
	create_ubi_volume_do(&vol);
	if (ret)
		goto err;

	ret = write_ubi_volume_do(vol.name, buf, len, full_bytes);
	if (ret)
		pr_err("write volume %s with bytes %u full_bytes %u failed\n",
				vol.name, len, full_bytes);
	else
		pr_info("write volume %s with bytes %u full_bytes %u OK\n",
				vol.name, len, full_bytes);

err:
	return ret;
}
#endif /* CONFIG_SUNXI_PRIVATE_UBIDEV */
...
#if IS_ENABLED(CONFIG_SUNXI_PRIVATE_UBIDEV)
int rawnand_ubi_detach_mtd(void)
{
	if (ubi_detach_mtd_do())
		return -EINVAL;
