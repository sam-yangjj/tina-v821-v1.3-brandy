...
static int spi0_change_clk(unsigned int dclk_src_sel, unsigned int dclk)
{
...
#if defined(CONFIG_MACH_SUN8IW21) || defined(CONFIG_MACH_SUN300IW1)
	reg_val &= (~(0x3 << 16));
	...
	SPI_DBG("spi0 clk change to %d, SPI0_CLK_REG 0x%08x value is 0x%08x\n",
			dclk, sclk0_reg_adr, get_reg(sclk0_reg_adr));
...
static int spic0_clk_request(void)
{
	...
	/* reset */
	reg_val = get_reg(SUNXI_CCM_BASE + SPI0_CLK_RST_REG);
	reg_val &= (~(0x1U << SPI0_CLK_RST_SHIFT));
	reg_val |= (0x1U << SPI0_CLK_RST_SHIFT);
	set_reg(SUNXI_CCM_BASE + SPI0_CLK_RST_REG, reg_val);
	/* ahb clock gate */
	reg_val = get_reg(SUNXI_CCM_BASE + SPI0_CLK_GATINE_REG);
	reg_val &= (~(0x1U << SPI0_CLK_GATINE_SHIFT));
	reg_val |= (0x1U << SPI0_CLK_GATINE_SHIFT);
	set_reg(SUNXI_CCM_BASE + SPI0_CLK_GATINE_REG, reg_val);
	...
	defined(CONFIG_MACH_SUN8IW11) || defined(CONFIG_MACH_SUN20IW1) || \
	defined(CONFIG_MACH_SUN300IW1)
...
{
	/* CCM_SPI0_CLK_REG */
	u32 sclk0_reg_adr = (SUNXI_CCM_BASE + SPI0_CLK_REG);
	/* close clock */
...
static int spic0_wait_tc_end(void)
{
	...
	while(!(get_reg(SPI_ISR) & (0x1 << 12))) {
		if (!time_before(timer_get_us(), timeout)) {
