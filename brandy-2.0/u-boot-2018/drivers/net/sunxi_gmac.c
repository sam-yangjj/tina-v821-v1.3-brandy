...
#define SUNXI_SYSCFG_REG	SUNXI_SYSCTRL_BASE
#define SUNXI_GMAC_25M_CLK_REG	GMAC_25M_CLK_REG
#else
#define SUNXI_SYSCFG_REG	SUNXI_SRAMC_BASE
#define SUNXI_GMAC_25M_CLK_REG	0x0970
#endif
...
#define GMAC_RMII_MASK		0x00002000 /* BIT(13) */
...
static void sunxi_gmac_set_clk(struct sunxi_gmac *chip)
{
	u32 value;
#ifdef CONFIG_MACH_SUN300IW1
	/* deassert gmac */
	value = readl((void *)SUNXI_CCM_BASE + BUS_Reset0_REG);
	value |= BIT(BUS_Reset0_REG_HRESETN_GMAC_SW_OFFSET);
	writel(value, (void *)SUNXI_CCM_BASE + BUS_Reset0_REG);

	/* enable gmac clk */
	value = readl((void *)SUNXI_CCM_BASE + BUS_CLK_GATING1_REG);
	value |= BIT(BUS_CLK_GATING1_REG_GMAC_HBUS_EN_OFFSET);
	writel(value, (void *)SUNXI_CCM_BASE + BUS_CLK_GATING1_REG);

	value = readl((void *)SUNXI_CCM_BASE + BUS_CLK_GATING0_REG);
	value |= BIT(BUS_CLK_GATING0_REG_GMAC_HCLK_EN_OFFSET);
	writel(value, (void *)SUNXI_CCM_BASE + BUS_CLK_GATING0_REG);
	...
	if (chip->phy_clk_type == SUNXI_PHY_USE_CLK25M) {
		 ...
		 */
		value = 0;
		value |= GMAC_25M_CLK_REG_GMAC_25M_CLK_SEL_CPUPLL << GMAC_25M_CLK_REG_GMAC_25M_CLK_SEL_OFFSET;
		value |= 19 << GMAC_25M_CLK_REG_GMAC_25M_CLK_DIV1_OFFSET;
		value |= 1 << GMAC_25M_CLK_REG_GMAC_25M_CLK_DIV2_OFFSET;
		value |= BIT(GMAC_25M_CLK_REG_GMAC_25M_CLK_EN_OFFSET);

		writel(value, (void *)SUNXI_CCM_BASE + SUNXI_GMAC_25M_CLK_REG);
	}
#else
	/* enable gmac clk */
	...
	/* Open phy25m clk */
	if (chip->phy_clk_type == SUNXI_PHY_USE_CLK25M) {
		value = readl(SUNXI_CCM_BASE + SUNXI_GMAC_25M_CLK_REG);
		value |= (3 << (GMAC_25M_GATE_BIT - 1));
		writel(value, SUNXI_CCM_BASE + SUNXI_GMAC_25M_CLK_REG);
	}
#endif
}
...
static int sunxi_gmac_hardware_init(struct sunxi_gmac *chip)
{
	...
	value = readl((void *)SUNXI_SYSCFG_REG + SYSCFG_PHY_CLK);
	...
	writel(value, (void *)SUNXI_SYSCFG_REG + SYSCFG_PHY_CLK);

	sunxi_gmac_set_clk(chip);
...
static int sunxi_gmac_send(struct eth_device *dev, void *packet, int length)
{
	...
	tx_p->desc3 = (u32)tx_p;	/* tx no need for ring buffer */

	wmb();

	tx_p->desc0.all = GMAC_DMA_OWN_DESC;  /* Set Own */

	flush_cache((unsigned long)tx_p, ALIGN(sizeof(struct dma_desc), CACHE_LINE_SIZE));
	flush_cache((unsigned long)packet, ALIGN(length, CACHE_LINE_SIZE));
...
static void sunxi_gmac_rx_refill(struct sunxi_gmac *chip)
{
	struct dma_desc *desc = chip->dma_desc_rx;

	while (circ_space(chip->rx_clean, chip->rx_dirty, RING_SIZE) > 0) {
		int entry = chip->rx_clean;

		/* Find the dirty's desc and clean it */
		desc = chip->dma_desc_rx + entry;
		desc->desc1.all = GMAC_RX_DMA_INT_CTL;
		desc->desc1.all |= GMAC_RX_DMA_BUFF_SIZE_MASK;
		desc->desc2 = (u32)chip->rx_packet[entry];

		wmb();

		desc->desc0.all = GMAC_DMA_OWN_DESC;
		chip->rx_clean = circ_inc(chip->rx_clean, RING_SIZE);
		flush_cache((unsigned long)desc, ALIGN(sizeof(*desc), CACHE_LINE_SIZE));
	}
}
...
static int sunxi_gmac_recv(struct eth_device *dev)
{
	...
	while (i <= RING_SIZE) {
		...
		i++;
		chip->rx_dirty = circ_inc(chip->rx_dirty, RING_SIZE);
		invalidate_dcache_range((ulong)(chip->rx_packet[entry]), (ulong)((ulong)(chip->rx_packet[entry]) + ALIGN(SZ_2K, CACHE_LINE_SIZE)));
		...
		if (recv_stat != discard_frame) {
			...
			pkt_hex_dump("RX", (void *)chip->rx_packet[entry], 64);

			net_process_received_packet((uchar *)(chip->rx_packet[entry]), len);
		} else {
...
static void sunxi_gmac_desc_init_chain(struct dma_desc *desc)
{
	int i;
	struct dma_desc *p = desc;
	u32 dma_phy = (u32)desc;
	u32 dma_phy_tmp = (u32)desc;
...
static int sunxi_gmac_init(struct eth_device *dev, bd_t *bis)
{
	...
	memset((void *)chip->dma_desc_tx, 0, sizeof(struct dma_desc) * RING_SIZE);
	memset((void *)chip->dma_desc_rx, 0, sizeof(struct dma_desc) * RING_SIZE);

	sunxi_gmac_desc_init_chain(chip->dma_desc_tx);
	sunxi_gmac_desc_init_chain(chip->dma_desc_rx);
	chip->tx_clean = 0;
	chip->rx_clean = 0;
	chip->tx_dirty = 0;
	chip->rx_dirty = 0;
...
static int sunxi_gmac_alloc_rx_packet(struct sunxi_gmac *chip)
{
	int i, ret = 0;

	for (i = 0; i < RING_SIZE; i++) {
		chip->rx_packet[i] = (unsigned char *)memalign(CACHE_LINE_SIZE, SZ_2K);
		memset(chip->rx_packet[i], 0, SZ_2K);
		if (!chip->rx_packet[i])
			ret = -ENOMEM;
	}
...
static void sunxi_gmac_free_rx_packet(struct sunxi_gmac *chip)
{
	int i;

	for (i = 0; i < RING_SIZE; i++) {
		if (chip->rx_packet[i])
			free(chip->rx_packet[i]);
	}
}
...
int sunxi_gmac_initialize(void)
{
	...
	chip->dma_desc_tx = (struct dma_desc *)
		memalign(CACHE_LINE_SIZE, sizeof(struct dma_desc) * RING_SIZE);
	if (!chip->dma_desc_tx) {
		ret = -ENOMEM;
		goto err_dma_tx;
	}
	...
	chip->rx_packet = (unsigned char **)memalign(CACHE_LINE_SIZE, sizeof(chip->rx_packet[0]) * RING_SIZE);
	if (!chip->rx_packet) {
		ret = -ENOMEM;
		goto err_rx_packet;
	}
	...
	ret = sunxi_gmac_alloc_rx_packet(chip);
	if (ret)
		goto err_alloc_rx_packet;
...
err_alloc_rx_packet:
	sunxi_gmac_free_rx_packet(chip);
err_hardware_init:
	/* nothing to do */
err_resource_get:
	free(chip->rx_packet);
