...
static u32 char_to_hex(char input)
{
	if (input >= 'a' && input <= 'f')
		return (input - 'a' + 10);
...
#endif
			} else if (!strncmp(key_list->name, "wmac_addr", (((u32)sizeof("wmac_addr")) - 1))) {
				u8 key_buf[6] = {0};
				for (int i = 0; i < 6; i++) {
					u32 mac_val1 = char_to_hex(((key_list->key_data)[i * 3]));
					u32 mac_val2 = char_to_hex(((key_list->key_data)[i * 3 + 1]));
					key_buf[i] = ((mac_val1 & 0x0F) << 4) | ((mac_val2) & 0x0F);
				}
				memcpy(key_list->key_data, key_buf, 6);
				efuse_key_info.len = 6;
			} else if (!strncmp(key_list->name, "wmac_sel", (((u32)sizeof("wmac_sel")) - 1))) {
				u8 key_buf;
				u32 mac_val1 = char_to_hex(((key_list->key_data)[0]));
				u32 mac_val2 = char_to_hex(((key_list->key_data)[1]));
				key_buf = ((mac_val1 & 0x0F) << 4) | ((mac_val2) & 0x0F);
				memcpy(key_list->key_data, &key_buf, 1);
				efuse_key_info.len = 1;
			}
...
#elif (IS_ENABLED(CONFIG_RISCV) && IS_ENABLED(CONFIG_SUNXI_SBI_ECALL))
			if (sbi_ecall_efuse_write(&efuse_key_info)) {
				return -1;
			}
#else
			if (sunxi_efuse_write(&efuse_key_info)) {
...
int __sunxi_read_key_by_name(void *buffer, uint buff_len, int *read_len)
{
	...
	*read_len = (u32)sizeof(sunxi_usb_burn_key_info_t);
...
#elif (IS_ENABLED(CONFIG_RISCV) && IS_ENABLED(CONFIG_SUNXI_SBI_ECALL))
		if (sbi_ecall_efuse_read(key_info->name, (void *)data_buff, &key_data_len)) {
			printf("efuse read %s err\n", key_info->name);
			return -1;
		}
	...
	else
	{
		...
		extern int key_extraction_output_generate(char *out, size_t out_size);
		extern int key_widevine_output_generate(char *out, size_t out_size);
		...
		if (strcmp("key-extraction", key_info->name) == 0) {
			...
			*read_len += strlen((char *)key_info->key_data) + 1;
			return ret;
		} else if (strcmp("key-widevine", key_info->name) == 0) {
			int ret = key_widevine_output_generate(
					(char *)key_info->key_data,
					buff_len - *read_len);
			key_info->len = strlen((char *)key_info->key_data) + 1;
			*read_len += strlen((char *)key_info->key_data) + 1;
...
static int sunxi_pburn_state_loop(void  *buffer)
{
	...
	switch(sunxi_usb_pburn_status)
	{
		...
		case SUNXI_USB_PBURN_EXIT:
			...
			if (cbw->CBWCDB[1] == USB_BURN_SET_NEXTWORK_REBOOT) {
