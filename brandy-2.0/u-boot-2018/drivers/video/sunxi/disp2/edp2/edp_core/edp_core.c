...
bool edp_source_support_enhance_frame(u32 sel)
{
	return edp_hal_support_enhance_frame(sel);
}
...
s32 edp_dpcd_set_lane_para(u32 sel, struct edp_tx_core *edp_core)
{
	...
	ret = edp_core_aux_read(sel, DPCD_0101H, 1, &tmp_rx_buf[0]);
	if (ret)
		return ret;
...
s32 edp_link_cr_training(u32 sel, struct edp_tx_core *edp_core)
{
	...
	edp_core_aux_read(sel, DPCD_0100H, training_info_len, training_info);
...
s32 edp_core_enhance_frame_enable(u32 sel, bool enable)
{
	s32 ret = RET_OK;
	char tmp_rx_buf[16];

	ret = edp_hal_enhance_frame_enable(sel, enable);
	if (ret)
		return ret;
	...
	ret = edp_core_aux_write(sel, DPCD_0101H, 1,  &tmp_rx_buf[0]);
...
bool edp_core_check_controller_error(u32 sel)
{
	return edp_hal_check_controller_error(sel);
}
