...
s32 edp_misc_parse(u32 sel)
{
	s32 ret = -1;
	s32  value = 1;
	char primary_key[20], name[32];
	struct sunxi_edp_output_desc *desc = g_edp_info[sel].desc;

	sprintf(primary_key, "/soc/edp%d", sel);

	ret = disp_sys_script_get_item(primary_key, "edid_timings_prefer", &value, 1);
	if (ret == 1)
		g_edp_info[sel].edid_timings_prefer = value ? true : false;
	else
		g_edp_info[sel].edid_timings_prefer = false;

	ret = disp_sys_script_get_item(primary_key, "sink_capacity_prefer", &value, 1);
	if (ret == 1)
		g_edp_info[sel].sink_capacity_prefer = value ? true : false;
	else
		g_edp_info[sel].sink_capacity_prefer = false;

	ret = disp_sys_script_get_item(primary_key, "timings_fixed", &value, 1);
	if (ret == 1)
		g_edp_info[sel].timings_fixed = value ? true : false;
	else
		g_edp_info[sel].timings_fixed = false;

	ret = disp_sys_script_get_item(primary_key, "edp_training_param_type", &value, 1);
	if (ret == 1) {
		if (value >= 3)
			EDP_ERR("edp_training_param_type out of range!\n");
		else
			g_edp_info[sel].edp_core.training_param_type = value;
	} else
		g_edp_info[sel].edp_core.training_param_type = 0;

	/* FIXME: limit some senior panel support 90/120 hz, default enable. we seem can not handle
	 * dual screen with different fps well */
	ret = disp_sys_script_get_item(primary_key, "fps_limit_60", &value, 1);
	if (ret == 1)
	...
	if (desc->type == EDP_OUTPUT)
		g_edp_info[sel].edp_core.controller_mode = 0;
	else
		g_edp_info[sel].edp_core.controller_mode = 1;

	/* especially for inno edp1.3 phy, change controller's working mode
	 * for enhancing edp panel's compatibility, avoid AUX fail */
	ret = disp_sys_script_get_item(primary_key, "force_mode",  (int *)name, 2);
	if (ret == 2) {
		if (strcmp(name, "dp") == 0)
			g_edp_info[sel].edp_core.controller_mode = 1;
		else if (strcmp(name, "edp") == 0)
			g_edp_info[sel].edp_core.controller_mode = 0;
	}
...
s32 edp_phy_cfg_parse(u32 sel)
{
	...
	if (ret == 1) {
		if ((value != 0) && (value != 1)) {
			EDP_ERR("Out of range, select from: 0-center_mode 1-downspread_mode\n");
			edp_core->ssc_mode = 0;
		} else
...
void edp_parse_dpcd(u32 sel, char *dpcd_rx_buf)
{
	...
	if (dpcd_rx_buf[3] & EDP_DPCD_FAST_TRAIN_MASK) {
		...
		EDP_DRV_DBG("fast training:not require\n");
	} else {
		sink_cap->fast_train_support = false;
		EDP_DRV_DBG("fast training:require\n");
	}
...
s32 edp_resource_init(u32 sel)
{
	...
	sprintf(main_key, "/soc/edp%d", sel);
	...
	if (sel == 1)
		g_edp_info[sel].irq = disp_getprop_irq("/soc/edp1", "interrupts", 0);
	else
		g_edp_info[sel].irq = disp_getprop_irq("/soc/edp0", "interrupts", 0);
	EDP_INFO("g_edp_info[sel].irq = %d\n", g_edp_info[sel].irq);
				...
				const char *buf, size_t count)
{
	...
	if (err) {
		EDP_ERR("%s syntax error!\n", __func__);
		return count;
	}

	if ((val != 0x0) && (val != 0x10) && (val != 0x11) && (val != 0x110) && \
	    (val != 0x111) && (val != 0x1010) && (val != 0x1011)) {
		EDP_ERR("unavailable param, select from: 0x0/0x10/0x11/0x110/0x111/0x1010/0x1011 !\n");
		return count;
	}
				...
				const char *buf, size_t count)
{
	...
	if ((val != 0) && (val != 1) && (val != 2)) {
		EDP_ERR("training param type out of range, select from: 0/1/2\n");
		return count;
	}
				...
				const char *buf, size_t count)
{
	...
	if (separator == NULL) {
		...
		if (ssc_mode > 1) {
			EDP_ERR("ssc mode should select from: 0/1\n");
			return count;
		}
...
void edp_get_source_capacity(u32 sel)
{
	...
	src_cap->enhance_frame_support = edp_source_support_enhance_frame(sel);
}
...
/* soft reset controller when error occurs in uboot, such as ESD */
void edp_check_error_and_reset_hardware(void)
{
	u32 i = 0;

	for (i = 0; i < g_edp_num; i++) {
		if (edp_core_check_controller_error(i))
			edp_soft_reset(i);
	}
}
EXPORT_SYMBOL_GPL(edp_check_error_and_reset_hardware);

void edp2_update_param_to_kernel(void)
{
	struct edp_tx_core *edp_core = NULL;
	u32 i = 0;
	char primary_key[20];
	...
	for (i = 0; i < g_edp_num; i++) {
		if (g_edp_info[i].enable) {
			memset(primary_key, 0, sizeof(primary_key));
			sprintf(primary_key, "/soc/edp%d", i);
			node = fdt_path_offset(working_fdt, primary_key);
			if (node < 0) {
				EDP_WRN("Node %s not found\n", primary_key);
				continue;
			}

			edp_core = &g_edp_info[i].edp_core;

			if (fdt_setprop_u32(working_fdt, node, "edp_boot_x", edp_core->timings.x_res) < 0) {
				EDP_WRN("Failed to set property edp_boot_x\n");
			}

			if (fdt_setprop_u32(working_fdt, node, "edp_boot_y", edp_core->timings.y_res) < 0) {
				EDP_WRN("Failed to set property edp_boot_y\n");
			}
		}
	}
}
EXPORT_SYMBOL_GPL(edp2_update_param_to_kernel);
...
s32 edp_output_sw_enable_early(u32 sel)
{
	...
	/* sometimes edid checksum would fail because of PCB layout's SIPI,
	 * so we can have extra try to ensure get the right EDID date
	 */
	do {
		if (try_cnt > 3)
			break;
		edid = edp_edid_get(sel);
		if (edid == NULL)
			EDP_ERR("fail to read edid, retry_cnt:%d\n", try_cnt);
		try_cnt++;
	} while (edid == NULL);
...
s32 edp_output_enable(u32 sel)
{
	...
	if (edp_core->lane_para.bit_rate == 0 || edp_core->lane_para.lane_cnt == 0) {
		EDP_ERR("lane para is not parse from DPCD or dts, please check!\n");
//		goto ERR_PHY;
		/* use default lane para if there's no pannel connect */
		edp_core->lane_para.bit_rate = BIT_RATE_DEFAULT;
		edp_core->lane_para.lane_cnt = LANE_CNT_DEFAULT;
	}
	...
	if (sink_cap->enhance_frame_support && src_cap->enhance_frame_support)
		edp_core_enhance_frame_enable(sel, true);
...
s32 edp_output_disable(u32 sel)
{
	...
	if (sink_cap->enhance_frame_support && src_cap->enhance_frame_support)
		edp_core_enhance_frame_enable(sel, false);
	...
	if (g_edp_info[sel].vcc_regulator)
		disp_sys_power_disable(g_edp_info[sel].vcc_regulator);

	if (g_edp_info[sel].vdd_regulator)
		disp_sys_power_disable(g_edp_info[sel].vdd_regulator);
...
void edp_output_soft_reset(u32 sel)
{
	/* if edp is not enable, skip it to
	 * avoid disable the resource has not enabled yet
	 */
	if (g_edp_info[sel].enable) {
		edp_output_disable(sel);
		mdelay(5);
		edp_output_enable_early(sel);
		edp_output_enable(sel);
	}
}
...
struct sunxi_edp_output_desc edp_desc = {
	...
	.soft_reset     = edp_output_soft_reset,
};

s32 dp_output_init(u32 sel)
{
	struct edp_tx_core *edp_core;
	s32 ret;
...
s32 dp_output_enable(u32 sel)
{
	...
	if (edp_debug->hpd_mask) {
		if ((edp_debug->hpd_mask == 0x10) || (edp_debug->hpd_mask == 0x110)\
			...
			EDP_WRN("sink device unconnect virtual!\n");
			return RET_FAIL;
		}
...
void dp_output_soft_reset(u32 sel)
{
	struct edp_tx_core *edp_core = &g_edp_info[sel].edp_core;

	if (g_edp_info[sel].enable) {
		dp_output_plugout(sel);
		...
		if (g_edp_info[sel].vdd_regulator)
			disp_sys_power_enable(g_edp_info[sel].vdd_regulator);

		if (g_edp_info[sel].vcc_regulator)
			disp_sys_power_enable(g_edp_info[sel].vcc_regulator);

		edp_clk_enable(sel, true);

		edp_core_phy_init(sel, edp_core);

		dp_output_plugin(sel);

		dp_output_enable(sel);
	}
}
...
struct sunxi_edp_output_desc dp_desc = {
	...
	.soft_reset = dp_output_soft_reset,
};


static const struct sunxi_edp_output_match sunxi_edp_match[] = {
	{
...
s32 of_match_edp_output_type(u32 sel)
{
	char main_key[20], str[60];
	u32 i = 0;
	s32 ret = 0;

	sprintf(main_key, "/soc/edp%d", sel);
	ret = disp_sys_script_get_item(main_key, "compatible", (int *)str, 2);
	EDP_DRV_DBG("%s's compatible: %s\n", main_key, str);

	if (ret == 2) {
		for (i = 0; i < (sizeof(sunxi_edp_match) / sizeof(struct sunxi_edp_output_match)); i++) {
			if (strncmp(str, sunxi_edp_match[i].compatible, 60) == 0) {
				EDP_DRV_DBG("match compatible: %s\n", sunxi_edp_match[i].compatible);
				return i;
			}
		}
	}
...
s32 edp_disable(u32 sel)
{
	...
	if (!desc->disable) {
		EDP_WRN("output disable function missing!\n");
		return RET_FAIL;;
	}
...
s32 edp_enable(u32 sel)
{
	...
	if (!desc->enable) {
		EDP_WRN("output enable function missing!\n");
		goto OUT;
	}
...
static s32 edp2_probe(u32 sel)
{
	...
	match = of_match_edp_output_type(sel);
	if (match < 0) {
		EDP_ERR("Unable to match edp output type\n");
		ret = -1;
		goto OUT;
	}
	g_edp_info[sel].desc = sunxi_edp_match[match].desc;
	...
	edp_get_source_capacity(sel);

	if (g_edp_info[sel].desc->init) {
		ret = g_edp_info[sel].desc->init(sel);
		if (ret) {
...
s32 edp2_remove(u32 sel)
{
	...
	for (i = 0; i < g_edp_num; ++i) {
		...
		if (!IS_ERR_OR_NULL(g_edp_info[se;].vcc_regulator)) {
			disp_sys_power_disable(g_edp_info[i].vcc_regulator);
			regulator_put(g_edp_info[i].vcc_regulator);
		}
	}

	disp_edp_drv_deinit(sel);
	sysfs_remove_group(&edp_dev[sel]->kobj, &edp_attribute_group);
	device_destroy(edp_class[sel], devid[sel]);

	class_destroy(edp_class[sel]);
	cdev_del(edp_cdev[sel]);
