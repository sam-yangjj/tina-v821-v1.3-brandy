...
static void edp_controller_mode_init(u32 sel, u32 mode)
{
	u32 reg_val;
...
s32 edp_aux_i2c_read(u32 sel, s32 addr, s32 len, char *buf, bool retry)
{
	...
	for (i = 0; i < 4; i++) {
		reg_val[i] = readl(edp_base[sel] + REG_EDP_AUX_DATA0 + i * 0x4);
		udelay(20);
		EDP_LOW_DBG("[%s]: data: reg_val[%d] = 0x%x\n", __func__, i, reg_val[i]);
	}
...
s32 edp_hal_phy_init(u32 sel, struct edp_tx_core *edp_core)
{
	...
	edp_hpd_enable(sel);
	edp_mode_init(sel, 1);
	edp_controller_mode_init(sel, edp_core->controller_mode);
...
bool edp_hal_check_controller_error(u32 sel)
{
	...
	reg_val1 = readl(edp_base[sel] + REG_EDP_HPD_PLUG);
