...
s32 drv_disp_init(void)
{
	...
	node_offset = disp_fdt_nodeoffset(FDT_DISP_PATH);
	...
	para->mclk[DISP_MOD_DE_AHB] = of_clk_get(node_offset, counter);
	if (IS_ERR(para->mclk[DISP_MOD_DE_AHB])) {
		printf("fail to get clk for de\n");
	}
	counter++;

	para->mclk[DISP_MOD_DISP_SYS_AHB] = of_clk_get(node_offset, counter);
	if (IS_ERR(para->mclk[DISP_MOD_DISP_SYS_AHB])) {
		printf("fail to get clk for de\n");
	}
	counter++;

	para->mclk[DISP_MOD_DISP_SYS_MBUS] = of_clk_get(node_offset, counter);
	if (IS_ERR(para->mclk[DISP_MOD_DISP_SYS_MBUS])) {
		printf("fail to get clk for de\n");
	}
	counter++;
#endif
	...
	para->mclk[DISP_MOD_TCON_BUS] = of_clk_get(node_offset, counter);
	if (IS_ERR(para->mclk[DISP_MOD_TCON_BUS])) {
		printf("fail to get clk for de\n");
	}
	counter++;
#endif
