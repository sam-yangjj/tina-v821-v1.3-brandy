...
static s32 lcd_parse_panel_para(u32 disp, disp_panel_para *info, int compat_idx)
{
	...
	if (info->lcd_if == 3) {
		...
		ret = lcd_sys_script_get_item(primary_key, "lvds0_lane_np_inv", &value, 1, compat_idx);
		if (ret == 1)
			info->lvds0_lane_np_inv = value;

		ret = lcd_sys_script_get_item(primary_key, "lvds1_lane_np_inv", &value, 1, compat_idx);
		if (ret == 1)
			info->lvds1_lane_np_inv = value;

		ret = lcd_sys_script_get_item(primary_key, "lvds_even_odd_dir", &value, 1, compat_idx);
		if (ret == 1)
			info->lvds_even_odd_dir = value;
		...
		ret = lcd_sys_script_get_item(primary_key, "lcd_hs_trail_set", &value, 1, compat_idx);
		if (ret == 1)
			info->hs_trail_set = value;
		else
			info->hs_trail_set = 4;
	...
	ret = lcd_sys_script_get_item(primary_key, "lcd_hv_de_polarity", &value, 1, compat_idx);
	if (ret == 1)
		info->lcd_hv_de_polarity = value;
...
#if !defined(CONFIG_MACH_SUN55IW3) && !defined(CONFIG_MACH_SUN300IW1)
...
static s32 lcd_clk_enable(struct disp_device* lcd)
{
	...
	if (lcdp->clk_tcon_bus) {
		ret = clk_prepare_enable(lcdp->clk_tcon_bus);
		if (0 != ret) {
...
static s32 disp_lcd_pin_cfg(struct disp_device *lcd, u32 bon)
{
	...
	if (g_compat_index > 0)
		...
		sprintf(dev_name, "/soc/lcd%d", lcd->disp);
...
s32 disp_init_lcd(disp_bsp_init_para * para)
{
	...
	for (hwdev_index=0; hwdev_index<num_devices; hwdev_index++) {
		...
		if (para->mclk[DISP_MOD_TCON_BUS])
			lcdp->clk_tcon_bus = para->mclk[DISP_MOD_TCON_BUS];
#endif
