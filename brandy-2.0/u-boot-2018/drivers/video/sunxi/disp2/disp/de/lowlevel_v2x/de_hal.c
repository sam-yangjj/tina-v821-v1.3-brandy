		...
		struct scaler_para *ovl_cpara)
{
	...
	for (j = 0, k = 0; j < chn; j++) {
		layno = de_feat_get_num_layers_by_chn(screen_id, j);
		for (i = 0; i < layno;) {
 ...
 *blending size, layer coordinate */
	...
	for (j = 0; j < chn; j++) {
		...
		for (i = 0; i < layno; i++) {
			if (lay_en[j][i] &&
			    ((ovlw[j] != bld_rect[j].w) || (ovlh[j] != bld_rect[j].h))) {
#if defined(CONFIG_MACH_SUN300IW1)
				if (!de_feat_is_support_scale_by_chn(screen_id, j)) {
					__inf("channal%d not support scale, "
					       "please check the size parameters!!!", j);
				}
#endif
			}
		}
...
int de_al_init(disp_bsp_init_para *para)
{
	...
	reg_base = para->reg_base[DISP_MOD_DE];
	return 0;
}

#if defined SUPPORT_AMP_PREVIEW_REG
void de_get_amp_preview_info(u32 disp, u32 chn, struct disp_amp_preview_channel_info *p)
{
	unsigned long ovl_base = reg_base + 0x00102000;
	u32 vcount = de_feat_get_num_vi_chns(disp);

	if (disp)
		ovl_base += 0x100000;

	p->alpha_reg = ovl_base + chn * 0x1000;
	p->alpha_shift = 24;
	p->addr_reg = p->alpha_reg + (chn < vcount ? 0x18 : 0x10);
	p->fill_color_en_reg = p->alpha_reg;
	p->fill_color_en_bit = 4;
	p->fill_color_reg = p->fill_color_en_reg;
	p->fill_color_reg += (chn < vcount ? 0xc0 : 0x18);
	p->layer_reg_size = chn < vcount ? 0x30 : 0x20;
}
#endif
