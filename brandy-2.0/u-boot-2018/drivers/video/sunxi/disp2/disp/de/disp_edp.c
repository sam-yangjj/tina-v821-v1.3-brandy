...
struct disp_edp_private_data {
	...
	bool edp_bl_en_used;
	bool edp_bl_pwr_en_used;
	bool edp_panel_pwr_en_used[EDP_POWER_NUM];
	bool edp_gpio_used[EDP_GPIO_NUM];
	disp_gpio_set_t edp_bl_en;
	disp_gpio_set_t edp_bl_pwr_en;
	disp_gpio_set_t edp_panel_pwr_en[EDP_POWER_NUM];
	disp_gpio_set_t edp_gpio[EDP_GPIO_NUM];
	int edp_bl_en_hdl;
	int edp_bl_pwr_en_hdl;
	int edp_panel_pwr_en_hdl[EDP_POWER_NUM];
	int gpio_hdl[EDP_GPIO_NUM];
...
/* param update to kernel and hwc */
static u32 edp_de_id;
static u32 edp_x_res;
static u32 edp_y_res;
...
static struct disp_video_timings default_timings = {
	.vic = 0,
...
static void disp_edp_save_param(u32 de_id, u32 x_res, u32 y_res)
{
	edp_de_id = de_id;
	edp_x_res = x_res;
	edp_y_res = y_res;
...
void edp_update_param_to_kernel(void)
{
	int node;
	char *standard_name = "EDP";
	char *name = NULL;
	char path_name[32] = {0};
	char key_name[32] = {0};
	int i = 0;

	if (!edp_x_res || !edp_y_res) {
		tick_printf("edp has not enable, update to kernel fail!\n");
		return;
	}
	...
	if (fdt_getprop_string(working_fdt, node, "primary_display_type", &name) >= 0) {
		if (strncmp(name, standard_name, strlen(standard_name)) == 0) {
			fdt_setprop_u32(working_fdt, node, "primary_framebuffer_width", edp_x_res);
			fdt_setprop_u32(working_fdt, node, "primary_framebuffer_height", edp_y_res);
		}
	}
	...
	for (i = 0; i < 4; i++) {
		...
		name = NULL;
		snprintf(path_name, sizeof(path_name), "extend%d_display_type", i);
		if (fdt_getprop_string(working_fdt, node, path_name, &name) >= 0) {
			if (strncmp(name, standard_name, strlen(standard_name)) == 0) {
				memset(key_name, 0, sizeof(key_name));
				snprintf(key_name, sizeof(key_name), "extend%d_framebuffer_width", i);
				fdt_setprop_u32(working_fdt, node, key_name, edp_x_res);

				memset(key_name, 0, sizeof(key_name));
				snprintf(key_name, sizeof(key_name), "extend%d_framebuffer_height", i);
				fdt_setprop_u32(working_fdt, node, key_name, edp_y_res);
			}
		}
	}

	memset(key_name, 0, sizeof(key_name));
	snprintf(key_name, sizeof(key_name), "fb%d_width", edp_de_id);
	fdt_setprop_u32(working_fdt, node, key_name, edp_x_res);

	memset(key_name, 0, sizeof(key_name));
	snprintf(key_name, sizeof(key_name), "fb%d_height", edp_de_id);
	fdt_setprop_u32(working_fdt, node, key_name, edp_y_res);
}
...
static s32 disp_edp_power_enable(struct disp_device *edp, u32 power_id)
{
	...
	if (edpp->edp_panel_pwr_en_used[power_id]) {
		memcpy(&gpio_info, &(edpp->edp_panel_pwr_en[power_id]),
			sizeof(disp_gpio_set_t));
		edpp->edp_panel_pwr_en_hdl[power_id] = disp_sys_gpio_request_simple(&gpio_info, 1);
	}
...
static s32 disp_edp_power_disable(struct disp_device *edp, u32 power_id)
{
	...
	if (edpp->edp_panel_pwr_en_used[power_id])
		disp_sys_gpio_release(edpp->edp_panel_pwr_en_hdl[power_id], 2);
...
s32 edp_regulator_get_wrap(struct disp_edp_private_data *edpp)
{
	...
	sprintf(primary_key, "/soc/edp%d", edpp->edp_index);
	...
	for (i = 0; i < EDP_POWER_NUM; i++) {
		if (edpp->edp_panel_regulator[i] == NULL) {
			if (edpp->edp_power_used[i]) {
				...
				DE_INF("edpp->edp_bl_regulator:%s\n", edpp->edp_bl_regulator);
				DE_INF("edpp->edp_panel_regulator:%s\n", edpp->edp_panel_regulator[i]);
			}
		}
	}
			 ...
			 struct disp_video_timings *p_info)
{
	...
	sprintf(primary_key, "/soc/edp%d", p_edpp->edp_index);
	...
	if (ret != 1 || value == 0) {
		...
		p_edpp->edp_bl_pwr_en_used = 0;

		gpio_info = &(p_edpp->edp_bl_pwr_en);
		ret = disp_sys_script_get_item(primary_key, "edp_bl_pwr_en",
					       (int *)gpio_info, 3);
		if (ret == 3)
			p_edpp->edp_bl_pwr_en_used = 1;

		for (i = 0; i < EDP_POWER_NUM; i++) {
			p_edpp->edp_panel_pwr_en_used[i] = 0;
			gpio_info = &(p_edpp->edp_panel_pwr_en[i]);

			sprintf(sub_name, "edp_panel_pwr_%d", i);
			ret = disp_sys_script_get_item(primary_key, sub_name,
						       (int *)gpio_info, 3);
			if (ret == 3) {
				p_edpp->edp_panel_pwr_en_used[i] = 1;
			}
		}
...
s32 disp_edp_enable(struct disp_device *p_edp)
{
	...
	if ((p_edpp->video_info == NULL) || (p_edpp->video_info->pixel_clk == 0)) {
		...
		/* if panel is not connect in some factory scene, use default timings */
		memcpy(p_edpp->video_info, &default_timings, sizeof(struct disp_video_timings));
//		goto EXIT;
	}
	...
	disp_edp_save_param(mgr->disp, p_edp->timings.x_res, p_edp->timings.y_res);
...
static s32 disp_edp_backlight_enable(struct disp_device *edp)
{
	...
	if (edpp->edp_bl_pwr_en_used) {
		memcpy(&gpio_info, &(edpp->edp_bl_pwr_en),
			sizeof(disp_gpio_set_t));
		edpp->edp_bl_pwr_en_hdl = disp_sys_gpio_request_simple(&gpio_info, 1);
	}
...
static s32 disp_edp_backlight_disable(struct disp_device *edp)
{
	...
	if (edpp->edp_bl_pwr_en_used)
		disp_sys_gpio_release(edpp->edp_bl_pwr_en_hdl, 2);
...
s32 disp_init_edp(disp_bsp_init_para *para)
{
	...
	for (hwdev_index = 0; hwdev_index < num_devices; hwdev_index++) {
		...
		sprintf(primary_key, "/soc/edp%d", disp);
...
//		p_edpp->reset = para->rst_bus_tcon[hwdev_index];
		...
		p_edp->get_preview_device_info = disp_device_get_preview_device_info;
