...
s32 lvds_open(u32 sel, disp_panel_para *panel)
{
	...
	if (panel->lcd_lvds_if == LCD_LVDS_IF_DUAL_LINK) {
		...
		/* SET tcon lvds ana0*/
		lcd_dev[sel]->tcon0_lvds_ana[0].bits.plr = (panel->lvds1_lane_np_inv)&0x0f;
		lcd_dev[sel]->tcon0_lvds_ana[0].bits.plrc = (panel->lvds1_lane_np_inv)>>4;
#endif
...
s32 tcon0_cfg(u32 sel, disp_panel_para *panel)
{
	if ((panel->lcd_if == LCD_IF_HV) || (panel->lcd_if == LCD_IF_EXT_DSI)) {
		...
		lcd_dev[sel]->tcon0_lvds_ctl.bits.tcon0_lvds_even_odd_dir = panel->lvds_even_odd_dir;
	...
	lcd_dev[sel]->tcon0_io_pol.bits.de_inv = panel->lcd_hv_de_polarity;
