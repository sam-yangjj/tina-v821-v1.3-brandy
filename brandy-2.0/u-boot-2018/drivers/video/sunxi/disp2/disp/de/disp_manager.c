...
struct disp_manager_private_data {
	...
	struct clk *dpss_clk;
#endif
	struct clk *de_ahb_clk;
	struct clk *disp_ahb_clk;
	struct clk *disp_mbus_clk;
	bool sync;
	bool err;
...
#if defined(CONFIG_AW_DISP2_SET_CLK_BY_IC_VERSION)
		unsigned int ic_ver = readl(0x03000024) & 0x00000007;
		...
		if (de_freq != clk_get_rate(mgrp->clk)) {
			...
			if (de_freq != clk_get_rate(mgrp->clk)) {
				DE_WRN("set DE clk fail,"
					"de_freq %ld real freq %ld",
					 de_freq, clk_get_rate(mgrp->clk));
				return -1;
			}
		}
	}
...
#endif
	if (mgrp->de_ahb_clk) {
		ret = clk_prepare_enable(mgrp->de_ahb_clk);
		if (ret != 0)
			DE_WRN("fail enable mgr's de_ahb_clk!\n");
	}
	if (mgrp->disp_ahb_clk) {
		ret = clk_prepare_enable(mgrp->disp_ahb_clk);
		if (ret != 0)
			DE_WRN("fail enable mgr's disp_ahb_clk!\n");
	}
	if (mgrp->disp_mbus_clk) {
		ret = clk_prepare_enable(mgrp->disp_mbus_clk);
		if (ret != 0)
			DE_WRN("fail enable mgr's disp_mbus_clk!\n");
	}
...
#endif
	if (mgrp->de_ahb_clk)
		clk_disable(mgrp->de_ahb_clk);
	if (mgrp->disp_ahb_clk)
		clk_disable(mgrp->disp_ahb_clk);
	if (mgrp->disp_mbus_clk)
		clk_disable(mgrp->disp_mbus_clk);
...
#if defined(HAVE_DISPLAY_SYS_TOP_CLK)
		if (para->mclk[DISP_MOD_DE_AHB])
			mgrp->de_ahb_clk = para->mclk[DISP_MOD_DE_AHB];
		if (para->mclk[DISP_MOD_DISP_SYS_AHB])
			mgrp->disp_ahb_clk = para->mclk[DISP_MOD_DISP_SYS_AHB];
		if (para->mclk[DISP_MOD_DE_AHB])
			mgrp->disp_mbus_clk = para->mclk[DISP_MOD_DISP_SYS_MBUS];
#endif
#if defined(CONFIG_INDEPENDENT_DE)
		if (disp == 0) {
