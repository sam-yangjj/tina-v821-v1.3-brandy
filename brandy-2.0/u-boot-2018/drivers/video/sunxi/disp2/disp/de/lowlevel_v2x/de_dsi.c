			  ...
			  __u32 lane)
{
	...
	n = DIV_ROUND_CLOSEST(frq_p, 24000000);
	if (m > 4) {
		printk("pclk is too small\n");
...
__s32 lvds_combphy_open(__u32 sel, disp_panel_para *panel)
{
	...
	/* SET LVDS_and_DSI0 COMBO DPHY*/
	dphy_dev[sel]->dphy_ana1.bits.reg_hst_plr = (panel->lvds0_lane_np_inv)&0x0f;
	dphy_dev[sel]->dphy_ana1.bits.reg_hxtsck_plr = (panel->lvds0_lane_np_inv)>>4;
