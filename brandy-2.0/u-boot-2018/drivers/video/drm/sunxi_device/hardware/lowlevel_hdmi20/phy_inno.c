...
{
	unsigned int id = 0;//sunxi_get_soc_markid();
	unsigned int version = 0;//sunxi_get_soc_ver();
...
int inno_phy_init(void)
{
	...
	/* parse dts */
	ret = ofnode_read_size(hdmi_node, "inno_phy");
	if (ret <= 0) {
	...
	ret = ofnode_read_u32_array(hdmi_node,
