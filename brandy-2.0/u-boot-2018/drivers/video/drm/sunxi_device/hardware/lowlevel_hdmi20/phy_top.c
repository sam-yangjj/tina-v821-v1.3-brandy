...
cfg_pll:
	...
	ret = dw_hdmi_wait_event_timeout(top_phy_pll_get_lock, 20);
	if (ret == 0) {
...
ssize_t top_phy_dump(char *buf)
{
	...
	n += sprintf(buf + n, " - state : [%s], [%s], [%s]\n",
		top_phy.rate_name,
		phy_reg->reg_0020.sun60i.pll_output_gate ? "enable" : "disable",
		top_phy_pll_get_lock() ? "lock" : "unlock");
	n += sprintf(buf + n, " - mpll  : 0x%08X, 0x%08X, 0x%08X, 0x%08X\n",
		phy_reg->reg_0020.dwval,
		phy_reg->reg_0028.dwval,
		phy_reg->reg_002C.dwval,
		phy_reg->reg_0030.dwval);
