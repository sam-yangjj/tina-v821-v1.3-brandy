...
int dw_video_use_hdmi20_vsif(void)
{
	...
	// prod->mOUI = HDMI_FORUM_IEEE_OUI; todo
...
ssize_t dw_avp_dump(char *buf)
{
	...
	n += sprintf(buf + n, " - timing   : [%03d] %d*%d%s@%dHz %s_%dbits\n",
		dw_avi_get_video_code(), hactive, dw_fc_video_get_vactive(),
	...
	if (state & IH_AS_STAT0_FIFO_OVERRUN_MASK)
		n += sprintf(buf + n, "     - fifo  : [over-run]\n");
	else if (state & IH_AS_STAT0_FIFO_UNDERRUN_MASK)
		n += sprintf(buf + n, "     - fifo  : [under-run]\n");
	else
		n += sprintf(buf + n, "     - fifo  : [ok]\n");

	n += sprintf(buf + n, " - [sample]\n");
	n += sprintf(buf + n, "     - freq  : [%dHz]\n",
			dw_fc_audio_get_sample_freq());
	n += sprintf(buf + n, "     - factor: [%dxFs]\n",
			_dw_audio_get_fs_factor());

	n += sprintf(buf + n, " - [acr]\n");
	n += sprintf(buf + n, "     - n     : [%d]\n",
			_dw_audio_get_clock_n());
	n += sprintf(buf + n, "     - cts   : [%d] [%s]\n",
