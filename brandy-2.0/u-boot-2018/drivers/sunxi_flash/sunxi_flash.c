...
#include "../../board/sunxi/board_helper.h"
...
int sunxi_sprite_download_priate(unsigned char *buf, int len)
{
	return sprite_flash->download_private_ubidev(buf, len);
}
...
int sunxi_flash_probe(void)
{
	...
	int storage_type = fdt_get_storage_type();
	if (storage_type >= 0) {
		switch (storage_type) {
		case STORAGE_NOR:
		     goto first_try_nor;
