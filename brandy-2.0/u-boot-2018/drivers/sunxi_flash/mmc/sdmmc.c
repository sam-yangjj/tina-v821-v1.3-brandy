...
{
#if !(defined(CONFIG_MACH_SUN50IW11) || defined(CONFIG_MACH_SUN300IW1))
	/* sprite to card2 */
...
{
	int ret = 0;
	struct mmc *mmc_type = mmc_sprite ? mmc_sprite : mmc_boot;

	if (mmc_type == NULL) {
		printf("error:sprite mmc is NULL\n");
		ret = -1;
		goto OUT;
	}
	pr_debug("use mmc(%s) for %s", (mmc_sprite ? "mmc_sprite" : "mmc_boot"), __func__);
	...
	if (mmc_type->block_dev.block_read(&mmc_type->block_dev,
		sunxi_flashmap_offset(FLASHMAP_SDMMC, SEC_STORAGE) + SDMMC_ITEM_SIZE * 2 * item,
		nblock, buf) != nblock) {
		printf("read first backup failed in fun %s line %d\n", __FUNCTION__, __LINE__);
		ret = -1;
