...
int sunxi_flash_nand_download_boot_param(void)
{
#ifdef CONFIG_SUNXI_UBIFS
	if (nand_use_ubi())
		return ubi_nand_download_boot_param();
#else
	printf("nand:unsupport downlaod boot_param\n");
#endif
	return 0;
}
...
static int
sunxi_flash_nand_download_private_ubidev(unsigned char *buf, int len)
{
#ifdef CONFIG_SUNXI_UBIFS
	if (nand_use_ubi())
		return ubi_nand_download_private_ubidev(len, buf);
#endif
	return -1;
}
#endif
...
sunxi_flash_desc sunxi_nand_desc =
{
    ...
    .download_boot_param = sunxi_flash_nand_download_boot_param,
	.download_toc = sunxi_flash_nand_download_toc,
	.upload_toc = sunxi_flash_nand_upload_toc,
	.write_end = sunxi_flash_nand_write_end,
#ifdef CONFIG_SUNXI_PRIVATE_UBIDEV
	.download_private_ubidev = sunxi_flash_nand_download_private_ubidev,
#endif
};
