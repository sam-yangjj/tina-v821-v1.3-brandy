...
{
	char buf[1024] = {0};
	void *base_addr = (void *)sspif->base_addr;
...
{
	return readl((void *)SUNXI_SPIF_BASE + SPIF_VER_REG);
}
...
static s32 sunxi_spif_gpio_request(void)
{
	...
	if (ret < 0) {
		SPIF_INF("set pin of spif fail %d\n", ret);
		return -1;
	}
...
void spif_init_dtr(u32 flags)
{
	...
	if ((sspif->mode & SPI_DTR_MODE) && (flags & USE_RX_DTR))
		sspif->rx_dtr_en = 1;
	else
		sspif->rx_dtr_en = 0;

	if ((sspif->mode & SPI_DTR_MODE) && (flags & USE_TX_DTR))
		sspif->tx_dtr_en = 1;
	else
		sspif->tx_dtr_en = 0;
}
...
int spif_set_right_delay_para(struct mtd_info *mtd, struct sunxi_boot_param_region *boot_param)
{
	struct spi_nor *nor = mtd->priv;
	struct spi_slave *slave = nor->spi;
	struct sunxi_spif_slave *sspif = to_sunxi_slave(slave);
