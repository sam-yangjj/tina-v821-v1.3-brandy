...
static __maybe_unused s32 sunxi_spi_gpio_request(unsigned int bus)
{
	int ret = 0;
	char spi_name[10];
...
static int sunxi_spi_gpio_init(unsigned int bus)
{
	...
	sunxi_gpio_set_cfgpin(SUNXI_GPC(6), SUNXI_GPC_SPI0); /*spi0_sclk */
	sunxi_gpio_set_cfgpin(SUNXI_GPC(7), SUNXI_GPC_SPI0); /*spi0_cs0*/

	sunxi_gpio_set_cfgpin(SUNXI_GPC(8), SUNXI_GPC_SPI0); /*spi0_mosi*/
	sunxi_gpio_set_cfgpin(SUNXI_GPC(9), SUNXI_GPC_SPI0); /*spi0_miso*/
	sunxi_gpio_set_cfgpin(SUNXI_GPC(10), SUNXI_GPC_SPI0); /*spi0_wp*/
	sunxi_gpio_set_cfgpin(SUNXI_GPC(11), SUNXI_GPC_SPI0); /*spi0_hold*/
...
static int sunxi_spi_clk_init(u32 bus, u32 mod_clk)
{
	...
	factor_m = m - 1;
#if defined(CONFIG_MACH_SUN8IW11) || defined(CONFIG_MACH_SUN300IW1)
...
#if defined(CONFIG_MACH_SUN8IW11) || defined(CONFIG_MACH_SUN300IW1)
	reset_shift = GATING_RESET_SHIFT;
	gating_shift = GATING_RESET_SHIFT;
	/* spi reset */
...
int sunxi_set_right_delay_para(struct mtd_info *mtd, struct sunxi_boot_param_region *boot_param)
{
	struct spi_nor *nor = mtd->priv;
	struct spi_slave *slave = nor->spi;
	struct sunxi_spi_slave *sspi = to_sunxi_slave(slave);
