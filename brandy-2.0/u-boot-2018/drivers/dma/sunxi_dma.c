...
void sunxi_dma_init(void)
{
	...
	setbits_le32(&ccm->mbus_gate, 1 << 2);

	/* dma reset */
	setbits_le32(&ccm->bus_clk_gating0_reg, 1 << 9);

	/* dma gating */
	setbits_le32(&ccm->dma_gate_reset, 1 << 9);
...
#elif defined(CONFIG_MACH_SUN300IW1)
	/* dma gating */
	clrbits_le32(&ccm->dma_gate_reset, 1 << 9);

	/* dma reset */
	clrbits_le32(&ccm->bus_clk_gating0_reg, 1 << 9);

	/* dma : mbus clock gating */
	clrbits_le32(&ccm->mbus_gate, 1 << 2);
#else
