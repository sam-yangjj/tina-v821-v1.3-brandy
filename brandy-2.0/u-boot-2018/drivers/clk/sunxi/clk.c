...
{
	int handle_num = 0;
	u32 handle[32] = { 0 }; // 32 is generally enough
	u32 parent_handle = 0;
	int i = 0;
	int ret = 0;
	char *clk_name = NULL;
...
void clk_summary_show(void)
{
	...
	printf("clock\t\t\t\t\t parent\t\t\t\t\t\t\t\t rate\n");
	/* search the 'proper' clk tree first */
	hlist_for_each_entry(c, pos, &clk_list, child_node) {
		pclk = clk_get_parent(c);
		if (pclk)
			printf("%-20s\t\t\t %-20s\t\t\t\t\t\t %lu\n", c->name, pclk->name, c->rate);
		else
			printf("%-20s\t\t\t NULL\t\t\t\t\t\t\t\t %lu\n", c->name, c->rate);
	}
}
