...
__weak void sunxi_plic_init(void)
{
...
#define RISCV_ARCH_IRQ_NUM	16
struct _irq_handler sunxi_rv_arch_handlers[RISCV_ARCH_IRQ_NUM];

void riscv_irq_install_handler (int irq, interrupt_handler_t handle_irq, void *data)
{
	if (irq >= RISCV_ARCH_IRQ_NUM || !handle_irq) {
		return;
	}

	sunxi_rv_arch_handlers[irq].m_data = data;
	sunxi_rv_arch_handlers[irq].m_func = handle_irq;
}

static void riscv_arch_irq_handler(uint irq_no)
{
	if (sunxi_rv_arch_handlers[irq_no].m_func != default_isr) {
		sunxi_rv_arch_handlers[irq_no].m_func(sunxi_rv_arch_handlers[irq_no].m_data);
	}
}
...
void timer_interrupt(struct pt_regs *regs)
{
	...
	riscv_mode == PRV_M ? csr_clear(mie, SR_MIE) : csr_clear(sie, SR_SIE);
	idnum = csr_read(scause) & 0x3ff;// bit 0~9
	if (idnum == IRQ_S_TIMER) {
		riscv_arch_irq_handler(idnum);
		writel(idnum, PLIC_M_S_CLAIM_REG);
	}
	riscv_mode == PRV_M ? csr_set(mie, SR_MIE) : csr_set(sie, SR_SIE);
	return;
