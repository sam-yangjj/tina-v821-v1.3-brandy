...
{
	writel(val, IOMEM_ADDR(SUNXI_RTC_DATA_BASE + index*4));
}

u32 rtc_read_data(int index)
{
	return readl(IOMEM_ADDR(SUNXI_RTC_DATA_BASE + index*4));
}

void rtc_set_fel_flag(void)
{
#ifdef CONFIG_MACH_SUN300IW1 /* for SUN300IW1 platform we need to set UPGRADE_FLAG to do fel */
...
#define SUNXI_UPGRADE_FLAG_CLEAR (0x429B0000)
	writel(SUNXI_UPGRADE_FLAG_SET, (void *) SUNXI_UPGRADE_FLAG_ADDR);
#endif
	do {
