...
void enable_interrupts(void)
{
	...
	if (cur_riscv_mode == PRV_S) {
		csr_set(sie, SIE_SEIE);
		csr_set(sstatus, SR_SIE);
	} else if (cur_riscv_mode == PRV_M) {
		csr_set(mie, EIE_EEIE);
		csr_set(mstatus, MIE_MSIE);
	}
}
...
int disable_interrupts(void)
{
	...
	if (cur_riscv_mode == PRV_S) {
		...
		return ((csr_read(sstatus) & SR_SIE) == 0);
	} else if (cur_riscv_mode == PRV_M) {
		csr_clear(mie, EIE_EEIE);
		csr_clear(mstatus, MIE_MSIE);
		return ((csr_read(mstatus) & MIE_MSIE) == 0);
	}
