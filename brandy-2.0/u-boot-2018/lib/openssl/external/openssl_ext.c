...
static int __asn1_probe_data_head(u8 *buf, sunxi_asn1_t *asn1)
{
	...
	//get len
	len = tmp_buf[1];
	if (len & 0x80) {//More than 1 byte indicates the length
		len_bytes = len & 0x7f;
...
static int __sunxi_publickey_dipatch(sunxi_key_t *pkey, u8 *buf, u32 len)
{
	...
	tmp_buf += asn1.head_len; //skip sequence head
	ret = __asn1_probe_data_head(tmp_buf, &asn1);
			       ...
			       u32 len)
{
	...
	// When calculating sha256, it is necessary to ensure 16-byte alignment at the start of memory.
	// The content to be signed in the certificate must not exceed 4k
	p_sign_to_calc = malloc_cache_aligned(4096);
	//Obtain the content to be signed
