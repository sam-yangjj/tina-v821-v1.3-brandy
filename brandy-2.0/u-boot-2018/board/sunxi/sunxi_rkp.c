...
int key_widevine_output_generate(char *out, size_t out_size)
{
	char fp_buf[128];
	__maybe_unused char base64_csr[4096];
	char tmp_csr[4096];
	...
	int ret;
	//cache
	if (uploaded_widevine) {
		strcpy(out, uploaded_widevine);
		return 0;
	}
	memset(tmp_csr, 0, sizeof(tmp_csr));
	uploaded_widevine = malloc(4096);
	//get finger print
	if (__get_finger_print(fp_buf)) {
		pr_err("get fingerprint fail\n");
		return -1;
	...
	pr_err("get fingerprint %s\n", fp_buf);
	//csr may from android or generate in place
	ret = sunxi_secure_object_read("drmkRkpCsr", tmp_csr, 4096, &tmp_csr_len);
	if (ret < 0) {
		printf("read widevine from android fail\n");
		return -1;
	} else {
		active_csr = tmp_csr;
	}
	...
	strncat(out, "\",\"csr\":\"", out_size - strlen(out) - 1);
	strncat(out, active_csr, out_size - strlen(out) - 1);
	strncat(out, "\",\"name\":\"widevine\" }", out_size - strlen(out) - 1);

	//"{\"build_fingerprint\":\"fp_buf\",\"csr\":\"active_csr\",\"name\":\"default\" }"

	//save csr cache
	strcpy(uploaded_widevine, out);
