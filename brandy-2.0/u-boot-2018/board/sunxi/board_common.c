...
void  __attribute__((weak)) sunxi_sid_enable_checksum(void)
{
}
...
int sunxi_burn_rotpk(void)
{
	...
	if (gd->securemode == SUNXI_NORMAL_MODE) {
		...
		sbi_ecall_efuse_read("rotpk", readback_hash, &hash_len);
#else
		sunxi_efuse_read("rotpk", readback_hash, &hash_len);
#endif
	}
...
#elif (IS_ENABLED(CONFIG_RISCV) && IS_ENABLED(CONFIG_SUNXI_SBI_ECALL))
		ret = sbi_ecall_efuse_write(&efuse_key_info);
		if (ret) {
			pr_err("svc burn rotpk failed with %d\n", ret);
			return -1;
		}
		if (sbi_ecall_efuse_read("rotpk", readback_hash, &hash_len)) {
			pr_err("svc read puk hash fail\n");
			return -1;
		}
#else
