...
int fdt_get_storage_type(void)
{
	...
	err = fdt_getprop_u32(working_fdt, nodeoffset, "storage_type",
			      (uint32_t *)&storage_type);
	if (err < 0) {
		printf("ftd get storage_type error\n");
		return -1;
	}
	return storage_type;
}
...
static int sunxi_update_fdt_rng_para(void)
{
	...
	sunxi_trng_gen(rng, 64);
	ret = fdt_setprop(working_fdt, nodeoffset, "kaslr-seed", rng, 64);
	if (ret < 0) {
		pr_err("update /chosen/kaslr-seed failed with %s\n", fdt_strerror(ret));
		return ret;
	}
...
int sunxi_update_fdt_para_for_kernel(void)
{
	...
	case STORAGE_NOR:
		...
		fdt_enable_node("/soc/spif0", 1);
		fdt_enable_node("/soc/spif0/spif-nor", 1);
		fdt_enable_node("/soc/spi0", 0);
		fdt_enable_node("/soc/spi0/spi_board0", 0);
#else
		fdt_enable_node("/soc/spi0", 1);
		fdt_enable_node("/soc/spi0/spi_board0", 1);
		fdt_enable_node("/soc/spif0", 0);
		fdt_enable_node("/soc/spif0/spif-nor", 0);
#endif
		break;
	case STORAGE_UFS:
