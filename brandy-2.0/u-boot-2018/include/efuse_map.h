...
#define EFUSE_WMAC_ADDR_NAME         "wmac_addr"
...
/* Refer to this struct when calling `sunxi_efuse_write()` */
typedef struct {
...
/* Unified API to write Efuse key map.
 * OTP bit one time burn feature: This API only cares about the unburned bits of `key_inf->key_data`.
 * So it is strongly suggested you read the value first, then set the unburned bits you want.
 * Param: @key_inf (Of type `efuse_key_info_t`). The size of `key_inf->key_data[]` should be a multiple of 4Byte.
 * Return val: 0 == success; negetive value is err number of type `efuse_err_e`
 */
int sunxi_efuse_write(void *key_inf);

/* Unified API to read Efuse key map.
 * Param: @key_name
 * Param: @rd_buf, should be aligned to 4Byte.
 * Return val: 0 == success; negetive value is err number of type `efuse_err_e`
 */
