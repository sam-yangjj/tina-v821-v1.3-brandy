...
uInt dictLength;
{
	...
	if (length > s->w_size) {
		...
		dictionary += dictLength -
			      length; /* use the tail of the dictionary */
	}
...
uLong sourceLen;
{
	...
	complen = sourceLen + ((sourceLen + 7) >> 3) + ((sourceLen + 63) >> 6) +
		  5;
		...
		if (s->wrap == 2) {
			...
			if (s->gzhead == Z_NULL) {
				...
				put_byte(s, (Byte)((s->gzhead->time >> 16) &
						   0xff));
				put_byte(s, (Byte)((s->gzhead->time >> 24) &
						   0xff));
				put_byte(
					s,
					s->level == 9 ?
		...
		if (bstate == block_done) {
			if (flush == Z_PARTIAL_FLUSH) {
				_tr_align(s);
				   ...
				   Z_BLOCK) { /* FULL_FLUSH or SYNC_FLUSH */
...
int ZEXPORT deflateEnd(strm) z_streamp strm;
{
	...
	if (status != INIT_STATE && status != EXTRA_STATE &&
...
#if MIN_MATCH != 3
				Call UPDATE_HASH(void) MIN_MATCH - 3 more times
#endif
