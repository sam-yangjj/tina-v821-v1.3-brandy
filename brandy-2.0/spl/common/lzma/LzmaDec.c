...
#define LzmaProps_GetNumProbs(p)                                               \
	((UInt32)LZMA_BASE_SIZE + (LZMA_LIT_SIZE << ((p)->lc + (p)->lp)))
	...
	do {
		...
		IF_BIT_0(prob)
		{
				...
				return SZ_ERROR_DATA;
			{
				...
				unsigned curLen =
					((rem < len) ? (unsigned)rem : len);
				SizeT pos = (dicPos - rep0) +
...
{
	UInt32 numProbs =
		Literal + ((UInt32)LZMA_LIT_SIZE << (p->prop.lc + p->prop.lp));
	UInt32 i;
	CLzmaProb *probs = p->probs;
