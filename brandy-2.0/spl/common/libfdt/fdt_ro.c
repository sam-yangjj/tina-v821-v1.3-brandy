...
const char *fdt_get_name(const void *fdt, int nodeoffset, int *len)
{
	...
	if (((fdt_check_header(fdt)) != 0) ||
	    ((fdt_check_node_offset_(fdt, nodeoffset)) < 0))
		goto fail;
			  ...
			  int namelen, int *lenp, int *poffset)
{
	for (offset = fdt_first_property_offset(fdt, offset); (offset >= 0);
		...
		const struct fdt_property *prop = fdt_get_property_by_offset_(fdt, offset, lenp);

		if (!prop) {
			offset = -FDT_ERR_INTERNAL;
			break;
		}
		if (fdt_string_eq_(fdt, fdt32_to_cpu(prop->nameoff), name,
				   namelen)) {
			if (poffset)
				*poffset = offset;
			return prop;
